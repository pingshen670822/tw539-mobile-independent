...
def _prediction_page(draws, weights, score, tickets, repeat_audit, ranking, target_date, generated_at, feature_labels, bt):
    ...
    content = f"""
<div class='band {'strong' if strong else 'primary'}'><div class='badge'>{confidence_label}</div><h2>本期最強1顆</h2><div class='number'>{ranking[0]:02}</div><p><b>已停用造成跨區間拖累的30期單碼重排；第1名直接採用穩定模型的正式首位，失準保護不得再換掉原始第1名。最後360期命中 {bt.get('single_hits',0)} 次，隔離命中率 {single_rate:.2f}%。</b></p><p class='note'>{'多重守門全部通過，列為超高信心強烈推薦。' if strong else '已產出本期綜合最強號碼；超高信心守門未全部通過，因此不偽造強烈推薦標籤。'}</p></div>
...
<div class='band strong'><h2>直接命中全排序校準</h2><p><b>固定最強1顆；其餘38顆以五組方向共識85%與直接命中模型15%融合，前9集合允許修正。最近54、120、360期的前5與前9，任何一項降低即禁止上線。</b></p><div class='grid'><div class='card'><div class='label'>最後360期前5</div><div class='value'>{direct_baseline.get('top5_avg_hits',0)} → {bt.get('top5_avg_hits',0)}</div></div><div class='card'><div class='label'>最後360期前9</div><div class='value'>{direct_baseline.get('top9_avg_hits',0)} → {bt.get('top9_avg_hits',0)}</div></div><div class='card'><div class='label'>最近120期前5／前9</div><div class='value'>{direct_baseline120.get('top5_avg_hits',0)}／{direct_baseline120.get('top9_avg_hits',0)} → {recent120.get('top5_avg_hits',0)}／{recent120.get('top9_avg_hits',0)}</div></div><div class='card'><div class='label'>最近54期前5／前9</div><div class='value'>{direct_baseline54.get('top5_avg_hits',0)}／{direct_baseline54.get('top9_avg_hits',0)} → {recent.get('top5_avg_hits',0)}／{recent.get('top9_avg_hits',0)}</div></div><div class='card'><div class='label'>最強1顆</div><div class='value'>完全不變</div></div><div class='card'><div class='label'>全排序融合占比</div><div class='value'>{100*bt.get('direct_hit_full_rank_blend',0):.0f}%</div></div></div></div>
...
<div class='band'><h2>正式方向模型</h2><p>每次預測與每一期回測都使用當時以前的全部歷史資料。先搜尋 {diagnostic.get('candidate_count',0)} 組錨定權重，保留 {diagnostic.get('eligible_candidate_count',0)} 組均衡候選，再建立 {bt.get('strategy_candidate_count',0)} 組模組正反方向模型；每一期只用此前 {bt.get('strategy_selection_window',0)} 期已開獎成績選出 {bt.get('strategy_consensus_member_count',0)} 組，平均成可完整驗算的正式權重。五組正式權重共識固定最強1顆，其餘38顆再用直接命中全排序校準，前9集合允許修正。</p><div class='table-wrap'><table><thead><tr><th>正式模組</th><th>資料來源</th><th>目前權重</th><th>方向</th></tr></thead><tbody>{formula_rows}</tbody></table></div></div>
<div class='band warning'><h2>全系統重組</h2><p><b>新版採用「五組方向共識＋直接命中全排序校準」雙層架構。</b>修正舊版只在錯誤前9集合內調位的重大缺陷；最強1顆固定不變，其餘38顆全部重新融合排序。最近54期前5／前9由 {direct_baseline54.get('top5_avg_hits',0)}／{direct_baseline54.get('top9_avg_hits',0)} 提高到 {(bt.get('recent_54') or {}).get('top5_avg_hits',0)}／{(bt.get('recent_54') or {}).get('top9_avg_hits',0)}；最近120期由 {direct_baseline120.get('top5_avg_hits',0)}／{direct_baseline120.get('top9_avg_hits',0)} 提高到 {(bt.get('recent_120') or {}).get('top5_avg_hits',0)}／{(bt.get('recent_120') or {}).get('top9_avg_hits',0)}；最後360期由 {direct_baseline.get('top5_avg_hits',0)}／{direct_baseline.get('top9_avg_hits',0)} 提高到 {bt.get('top5_avg_hits',0)}／{bt.get('top9_avg_hits',0)}。</p></div>
...
def _health_page(draws, bt, full_scan, generated_at, settlements, health):
    ...
    checks = (
        ...
        ("直接命中全排序校準", "通過" if bt.get("direct_hit_calibration_enabled") and bt.get("direct_hit_full_rank_gate") else "未通過", "固定最強1顆，其餘38顆重新融合；前9集合允許修正"),
