...
<div class='band'><h2>正式方向模型</h2><p>每次預測與每一期回測都使用當時以前的全部歷史資料。先搜尋 {diagnostic.get('candidate_count',0)} 組錨定權重，保留 {diagnostic.get('eligible_candidate_count',0)} 組均衡候選，再建立 {bt.get('strategy_candidate_count',0)} 組模組正反方向模型；每一期只用此前 {bt.get('strategy_selection_window',0)} 期已開獎成績選出 {bt.get('strategy_consensus_member_count',0)} 組，平均成可完整驗算的五組正式權重共識。其餘38顆再用直接命中全排序校準；第一名另經單碼重複冷卻守門；最後比較每期資料變化並固定前5後校正前9邊界。</p><div class='table-wrap'><table><thead><tr><th>正式模組</th><th>資料來源</th><th>目前權重</th><th>方向</th></tr></thead><tbody>{formula_rows}</tbody></table></div></div>
<div class='band warning'><h2>全系統重組</h2><p><b>新版採用「五組方向共識＋直接命中全排序校準＋單碼重複冷卻＋每期資料變化校正」四層架構。</b>修正舊版前9集合及第一名鎖死兩項缺陷。最近54期單碼由 {bt.get('single_repeat_break_recent_54_baseline_hits',0)} 中提高到 {bt.get('single_repeat_break_recent_54_hits',0)} 中，最近120期由 {bt.get('single_repeat_break_recent_120_baseline_hits',0)} 中提高到 {bt.get('single_repeat_break_recent_120_hits',0)} 中，最後360期由 {bt.get('single_repeat_break_baseline_hits',0)} 中提高到 {bt.get('single_repeat_break_hits',0)} 中。</p></div>
...
def _health_page(draws, bt, full_scan, generated_at, settlements, health):
    ...
    checks = (
        ...
        ("每期資料變化校正", "通過" if bt.get("data_change_enabled") and bt.get("data_change_gate") else "未通過", "固定前5，只校正第6名以後的前9邊界"),
