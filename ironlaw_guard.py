...
checks={
    ROOT/'.github/workflows/auto-update.yml':['push:','branches: [master]','report_pages.py','--strict-freshness','--structure-only','鐵律最終驗證','ironlaw_guard.py','issues: write','for attempt in 1 2 3','fetch-depth: 0','git checkout -B master origin/master','git branch --set-upstream-to=origin/master master','git fetch origin master','git rebase --abort || true','git rebase -X theirs origin/master','本地資料完整，立即略過重算並重新部署','部署後深度模型與版面診斷','timeout-minutes: 25','timeout-minutes: 18','site/mobile-sync.js','site/service-worker.js','actions/checkout@v7','actions/setup-python@v7','actions/configure-pages@v6','actions/upload-pages-artifact@v5','actions/deploy-pages@v5'],
    ROOT/'cloud_pipeline.py':['verify_freshness','verify_publication','expected_latest_date','prediction-history.jsonl','published-settlements.jsonl','compact_prediction_history','enrich_settlement','completed_from_pre_draw_seal','no_post_draw_substitution','rolling_adjustment','rank10_15_hits','boundary_review_status','boundary_discrimination_gap','polarity_model_candidate_count','polarity_selection_window','polarity_consensus_member_count','full_history_mode','replace=True','ranking_direction_valid','datetime.now(TAIPEI)','REPORT_PAGE_FILES','refresh_report_pages','two_hour_repair_deadline','self_repair_count','mobile_open_sync','MODEL_TIMEOUT_SECONDS=900','timeout=MODEL_TIMEOUT_SECONDS','subprocess.TimeoutExpired','FAST_WATCHDOG=','REQUEST_RETRY_DELAYS=(0,1) if FAST_WATCHDOG else (0,3,10)','REQUEST_TIMEOUT_SECONDS=8 if FAST_WATCHDOG else 45','apple-touch-icon','mobile-web-app-capable','icons/icon-180.png','icons/icon-192.png','icons/icon-512.png','icons/maskable-512.png'],
    ...
    ROOT/'system_audit.py':['整套系統驗收失敗','1中1主選未固定產出並公開','戰報可見英文','正式主選與隔離回測不是同一組權重','最新開獎錯誤沒有觸發三十組方向模型五組權重共識','命中檢討','三十組前9邊界參數','推薦牌組含強制投注排除號碼','tw539-mobile-ironlaw-v7','AbortController','SYNC_TIMEOUT_MS=10000','SUCCESS_SYNC_DELAY_MS=30000','RETRY_SYNC_DELAY_MS=5000','timer=setTimeout(checkVersion,nextDelay)','syncInFlight'],
    ROOT/'site/manifest.webmanifest':['"id": "./"','"scope": "./"','"display": "standalone"','icon-192.png','icon-512.png','maskable-512.png'],
    ROOT/'site/service-worker.js':["cache:'no-store'",'tw539-mobile-ironlaw-v7','system-health.json','backtest.html','review.html','history.html','models.html','health.html','mobile-sync.js','icons/icon-192.png','icons/icon-512.png','icons/maskable-512.png'],
    ROOT/'site/mobile-sync.js':['AbortController','SYNC_TIMEOUT_MS=10000','SUCCESS_SYNC_DELAY_MS=30000','RETRY_SYNC_DELAY_MS=5000','syncInFlight','timer=setTimeout(checkVersion,nextDelay)','visibilitychange','pageshow','online','同步正常','同步重試中','同步逾時，立即重試','beforeinstallprompt','appinstalled','install-app-button','手機版已安裝'],
    ROOT/'.github/workflows/watchdog.yml':['watchdog.py','auto-update.yml','repair=true','立即比較官方與手機公開頁','完整穩定性巡檢','TW539_WATCHDOG_FAST: 1',"steps.fast_check.outcome == 'failure' || steps.full_check.outcome == 'failure'",'for i in {1..40}','sleep 20','timeout-minutes: 25','timeout-minutes: 17','30,35,40,45,50,55 12','*/5 13-16','actions: write','actions/checkout@v7','actions/setup-python@v7'],
    ROOT/'watchdog.py':['FAST_WATCHDOG=','PUBLIC_FETCH_RETRY_DELAYS=(0,1) if FAST_WATCHDOG else (0,2,6)','PUBLIC_FETCH_TIMEOUT_SECONDS=5 if FAST_WATCHDOG else 25','即時自修檢查失敗','即時自修檢查','def fetch_public','tw539-mobile-ironlaw-v7','AbortController','SYNC_TIMEOUT_MS=10000','syncInFlight','system-health.json','published-settlements.jsonl','full_history_mode','latest_period','開獎後兩小時官方資料仍未取得','自主修復欄位','三十組方向模型、三百六十期選擇窗與五組權重共識','第10至15名偏移檢查','REPORT_PAGES','本期預測頁混入其他分類資料','開獎檢討分頁內容不完整'],
}
...
auto_text=(ROOT/'.github/workflows/auto-update.yml').read_text(encoding='utf-8')
if auto_text.find('actions/deploy-pages@v5')>auto_text.find('name: 部署後深度模型與版面診斷'):
    bad.append('自修部署仍被深度診斷阻擋')
watchdog_text=(ROOT/'.github/workflows/watchdog.yml').read_text(encoding='utf-8')
if '等待公開頁版本切換' in watchdog_text or 'sleep 30' in watchdog_text:
    bad.append('看門狗仍含自修啟動前固定等待')
