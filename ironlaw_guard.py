...
checks={
    ...
    ROOT/'tw539_ultra.py':['FORMAL_FEATURE_KEYS','GLOBAL_HISTORY_BLEND = 1.00','MODEL_SEARCH_CANDIDATE_COUNT = 286','MAX_ANCHOR_MODULE_WEIGHT = .50','ROLLING_ENSEMBLE_MEMBERS = 3','select_rolling_weights','select_rolling_learning_rate','rolling_weight_update','three_balanced_models_borda_then_post_draw_module_error_update','三模型名次共識','每期開獎命中檢討與滾動修正','禁止開獎後換號或補號','FEATURE_LABELS','短期視窗不得參與正式排名','all_available_history_for_every_prediction','history_coverage','"single_published": ranked[0]','bottom1_hits','bottom5_avg_hits','ranking_direction_valid','model_selection_cutoff','三模型回測終點必須等於畫面正式模型','強制投注排除名單','禁止進入任何推薦牌組','full_history_scan','model_score_with_repeat_qualification','連莊資格驗算','相對指數至少75','全歷史連莊率不低於12.82%','repeat_backtest_pass','不做補位','def rank_numbers','禁止固定偏向小號或大號'],
    ROOT/'system_audit.py':['整套系統驗收失敗','1中1主選未固定產出並公開','戰報可見英文','正式主選與隔離回測不是同一組權重','最新開獎錯誤沒有逐一回灌三個模型','命中檢討','推薦牌組含強制投注排除號碼'],
