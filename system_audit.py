...
from tw539_ultra import FORMAL_FEATURE_KEYS, GLOBAL_HISTORY_BLEND, MODEL_SEARCH_CANDIDATE_COUNT, MODEL_SELECTION_DATE, MODEL_SELECTION_PERIOD, MODEL_SELECTION_VALIDATION, apply_repeat_qualification, formal_history_state, load_draws, rank_numbers, ranking_direction_metrics, scores, scores_from_features, valid_ticket
...
if ranked!=rank_numbers(qualified_scores,latest['period'])[:15]: fail('連莊資格後排名與公開排名不同')
...
for path in (REPORTS/'最新539科學預測戰報.html',SITE/'index.html'):
    ...
    for term in ('1中1主選','公開狀態','已公開','全歷史核心占比','每期固定產出並公開','相對指數（非機率）','開獎前封存實戰紀錄','強制投注排除名單','禁止進入任何推薦牌組','全歷史逐期一致性掃描','上一期號碼檢查','連莊資格驗算','相對指數至少75','全歷史連莊率不低於12.82%','加權貢獻','不做補位'):
